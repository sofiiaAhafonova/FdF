...
void rotate(t_map *map)
{
	...
	while (++i < map->row)
	{
		...
		while (++j < map->col)
		{
			...
			map->offset[i][j].x = (int)round(map->offset[i][j].x * cos(map->wz) + map->offset[i][j].y * sin(map->wz));
			map->offset[i][j].y = (int)round(map->offset[i][j].y * cos(map->wz) - map->offset[i][j].x * sin(map->wz));
